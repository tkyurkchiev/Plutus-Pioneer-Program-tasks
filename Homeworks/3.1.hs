...
mkValidator dat () ctx = let 
    info = scriptContextTxInfo ctx
    sigs = txInfoSignatories info
    range = txInfoValidRange info
    in (unPaymentPubKeyHash (beneficiary1 dat) `elem` sigs) && (to       (deadline dat) `contains` range) ||
       (unPaymentPubKeyHash (beneficiary2 dat) `elem` sigs) && (from (1 + deadline dat) `contains` range)
